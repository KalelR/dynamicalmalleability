...
"""
Eg all Rstats for a single p
"""
function collectresults_singlefile(dictname::String, idx_file; data_dir)
    names_results, _= get_resultnames(dictname, quantifier, ""; data_dir = data_dir)
    name_result = names_results[idx_file]
    if(isfile(name_result) == false) @error("No file $(name_result)") end
    df = CSV.File(name_result, header=false) |> DataFrame
end
